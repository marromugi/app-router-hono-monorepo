...
export const Form = ({ id }: FormProps) => {
  ...
  const { trigger } = useMutation('/api/login', 'post', 200, {
    onError: async (err) => {
  ...
  const handleLogin = async (values: SchemaType<typeof schema>) => {
    await trigger({
      json: { ...values }
    })
  ...
  return (
    <Flex direction={'column'} gap={4}>
      <div className={clsx('mg-h-10')}>
        {error && (
          <Typography theme={'alert'} className={clsx('mg-animate-fade-in')}>
            {error}
          </Typography>
        )}
      </div>
      ...
      />
      <Button color={'gray'} disabled={!formState.isValid} onClick={handleSubmit(handleLogin)}>
        ログイン
      </Button>
    </Flex>
  )
