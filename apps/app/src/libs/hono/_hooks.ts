import { ClientRequest, ClientRequestOptions, type InferRequestType, type InferResponseType } from 'hono/client'
...
>(
  ...
  config: SWRConfiguration<InferResponseType<T['$get'], 200> | null, any> = {}
) => {
...
export const useMutation = <P extends Record<string, any>, M extends ExtractMutationMethod<keyof P>>(
  client: P,
  method: M,
  options: ClientRequestOptions<unknown> = {},
  config: SWRMutationConfiguration<Awaited<ReturnType<P[M]>>, any> = {}
) => {
  const key = `${client.$url()}-${method.toString()}`
  return useSWRMutation(
    key,
    (_key: string, { arg }: { arg: InferRequestType<P[M]> }) => {
